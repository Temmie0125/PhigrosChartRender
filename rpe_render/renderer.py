...
def _create_figure(columns: list[ColumnInfo], dpi: int) -> tuple[Figure, Axes, Axes]:
    ...
    ax_main.set_xlim(
        -SIDE_MARKER_PADDING_PX,
        columns[-1].pixel_right + SIDE_MARKER_PADDING_PX,
    )
...
def render(config: RenderConfig) -> None:
    ...
    主流程:
        1. 解析谱面 → ChartData
        2. 合并 Notes / 计算总拍数 / 分栏 / 统计
        3. 构建 NoteRenderInfo（多层 X 叠加计算 true_x）
        4. 创建画布 → 背景 → 网格 → 标记 → Hold → Note 贴图
        5. 渲染底部信息栏并保存 PNG
    """
    # ===== Phase 1: 解析 =====
    ...
    columns = compute_columns(max_beat)
    ...
    for line in chart.judge_line_list:
        for note in line.notes:
            ...
            true_x = judge_line_x_at(line, t_beat) + note.position_x  # ★ 多层叠加
            col, y_px, x_off = beat_to_pixel(t_beat, columns)
            x_px = x_to_pixel(true_x, x_off)
            _, y_px_end, _ = beat_to_pixel(note.end_time_beat, columns)
    ...
    canvas_w_px = columns[-1].pixel_right + 2 * SIDE_MARKER_PADDING_PX
    ...
    # zorder 映射：Hold Head/End 与普通 Note 按 startTime 统一排序
    sorted_infos = sorted(notes_info, key=lambda n: n.beat)
