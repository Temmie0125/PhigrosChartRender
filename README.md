...
- **下落式时间轴**：时间从下往上递增，默认按谱面长度自动调节每栏拍数（16–128，每 4 拍一档），超长谱面自动横向分栏
...
# 默认自动选择每栏拍数；也可固定为 16~128 且为 4 的倍数
...
render(RenderConfig(
    ...
    smart_column_beats=True,       # 按谱面长度自动调节每栏拍数，默认开启
    column_beats=64,               # 关闭自动调节时使用，范围 16~128，步长 4
    dpi=150,
